...
import { COOKIE_EXPIRE_TIME, MAX_LIKES_COUNT_PER_SESSION, MAX_LIKES_FROM_ONE_IP_ADDRESS } from "./constants";
  ...
  try {
    await prisma.sessionData.create({
      data: {
        sessionId: sessionId,
        ipAddress: IpAddress,
        expiresAt: new Date(Date.now() + COOKIE_EXPIRE_TIME),
      }
    });
  } catch (error) {
...
export async function canUserLikePhoto(sessionId) {
  try {
    ...
    if (user.photoLikes >= MAX_LIKES_COUNT_PER_SESSION) {
      console.warn(`User already liked: ${user.photoLikes}. Max: ${MAX_LIKES_COUNT_PER_SESSION} `)
...
export async function getCookieExpireTime(sessionId) {
  try {
    const user = await prisma.sessionData.findUnique({
      where: { sessionId: sessionId },
    });
...
export async function resetUserLikes() {
  try {
